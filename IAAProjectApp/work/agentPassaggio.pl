...
:-write('Sono un passaggio a livello!'),assertz(strada_libera(true)).

:-assertz(treno_prenotato(1,2,4)),assertz(treno_prenotato(2,1,6)),assertz(treno_prenotato(3,1,5)).

eve(richiesta_attraversamento(var_Id,var_Priorita)):-now(var_Timestamp),assertz(treno_prenotato(var_Id,var_Priorita,var_Timestamp)),format('il treno con id ~w ha chiesto di attraversare al tempo ~2f',[var_Id,var_Timestamp]),nl.

a(elabora_coda):-write('Elaboro la coda...'),nl,treno_prenotato(var__,var_Xp,var__),not((treno_prenotato(var__,var_Xp2,var__),var_Xp2>var_Xp)),treno_prenotato(var_Id,var_Xp,var_Xt),not((treno_prenotato(var__,var_Xp,var_Xt2),var_Xt2<var_Xt)),format('Treno successivo: ~d',var_Id),nl,retract(strada_libera(true)),assertz(strada_libera(false)),a(concedi_autorizzazione(var_Id)).

a(concedi_autorizzazione(var_Id)):-format('Ora il treno con id ~d puo passare',var_Id),nl,number_chars(var_Id,var_App),atom_chars(var_IdString,var_App),atom_concat('agentLocomotiva',var_IdString,var_AgentName),a(message(var_AgentName,propose(attraversamento,[],var_Me))).

eve(termino_attraversamento(var_Id)):-retract(treno_prenotato(var_Id,var__,var__)),format('Il treno con id ~d mi ha comunicato di aver terminato di attraversare',var_Id),nl,retract(strada_libera(false)),assertz(strada_libera(true)).

strada_libera:-write('').

evi(strada_libera):-strada_libera(var_X),true==var_X,treno_prenotato(var_Id,var_Priorita,var_Timestamp),a(elabora_coda).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_353423,var_Ontology,_353427),_353417),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_353461)),a(message(var_Ag,propose(var_A,[_353461],var_AgI))),retractall(ext_agent(var_Ag,_353499,var_Ontology,_353503)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_353297,var_Ontology,_353301),_353291),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_353367,var_Ontology,_353371)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_353185,var_Ontology,_353189),_353179),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_353241,var_Ontology,_353245)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_352749),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_352783),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_352565).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_352413),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_352361),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_352237,_352239)),clause(agente(_352259,_352261,_352263,var_S),_352255),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_352031,_352033,_352035,var_S),_352027),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_352131,[]),repeat,read(_352131,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_352131).
